...
class WM_Optimizer:
    def __init__(
        ...
        # 문자열로 전달된 경우 float로 변환
        self._lr = float(lr) if isinstance(lr, str) else lr
        self._eps = float(eps) if isinstance(eps, str) else eps
        self._clip = clip
        self._wd = wd
        self._wd_pattern = wd_pattern
        self._opt = {
            "adam": lambda: torch.optim.Adam(parameters, lr=self._lr, eps=self._eps),
            "nadam": lambda: NotImplemented(f"{opt} is not implemented"),
            "adamax": lambda: torch.optim.Adamax(parameters, lr=self._lr, eps=self._eps),
            "sgd": lambda: torch.optim.SGD(parameters, lr=self._lr),
            "momentum": lambda: torch.optim.SGD(parameters, lr=self._lr, momentum=0.9),
        }[opt]()
